...
def build_default_handlers(
    *,
    ...
    ``agent``, ``skill``, ``tool``, ``conditional``, and ``human`` are
    supported here; ``subflow``/``map`` arrive with E3-S5 — until then those
    node types fail closed via :class:`UnsupportedNodeError`.
