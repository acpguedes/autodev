...
from typing import Any, Dict, Iterable, List, Mapping, NamedTuple, NotRequired, Optional, Tuple, TypedDict
...
class _DispatchRecord(NamedTuple):
    """One dispatched task's first-attempt outcome, pending self-repair rendering (E46-S3).

    Accumulated across a batch's dispatch pass so :meth:`OrchestratorService._render_dispatch_records`
    can run one batched self-repair pass over every eligible task before
    appending any task's final ``AgentExecution``/``RunStep``/``HistoryItem``
...
class OrchestratorService:
    ...
    def _maybe_self_repair(
        ...
        *,
        ...
        ``_process_tasks`` itself calls :meth:`_maybe_batch_self_repair`
        directly so a batch with several failing validation tasks gets one
        Coder call, not one per task.
        """
