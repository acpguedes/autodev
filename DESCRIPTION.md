...
The **v2 platform** has completed its Beta wave and is published as the [`v2.0-beta`](https://github.com/acpguedes/autodev/releases/tag/v2.0-beta) pre-release. It delivers a small core surrounded by typed extension points: a Plugin Host with declarative `plugin.yaml` manifests, default-deny permissions and a Python SDK; an Agent Registry and budgeted Agent Runtime with `/v2` catalog APIs; a Flow Engine executing `flow.yaml` graphs with checkpointing, deterministic replay and human-in-the-loop nodes; pluggable reasoning strategies, routing/selection policies and an Evaluation Service with a closed feedback loop; skills as versioned extensions; tree-sitter + pgvector hybrid retrieval; a multi-tenant PostgreSQL state store with RLS, an Event Store and a MinIO Artifact Store; the Control Plane API `/v2` with run streaming and MCP interoperability; the Execution Control Center UI with step-level approval gates; real governed task execution in a fail-closed isolated environment with a policy engine, secret governance and full OpenTelemetry tracing, RBAC and per-tenant budgets; and a global `autodev` install with a version-compatibility-checked upgrade path.

Three Beta exit criteria remain **open** — a live-environment retrieval benchmark, a numeric run-streaming latency assertion, and a staging backup/restore validation — and are named rather than rounded up in `docs/v2_platform/beta_gap_analysis.md` §11. GA (Marketplace, E13) is not started. The **v1** linear-pipeline generation is frozen at the `v1` tag, with its documentation archived under `docs/archive/v1/`.
