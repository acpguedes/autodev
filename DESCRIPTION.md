...
The current implementation already includes a bootstrap durable control plane, persisted workflow-step history, configurable stub/OpenAI/Ollama agent execution, a persisted runtime configuration layer for LLM and repository/workspace selection, a dedicated frontend config workspace, a first repository-context retrieval API for ranked file discovery, post-analysis execution-plan generation with sequential task execution, a structured local CLI for operators, and published typed agent metadata contracts for downstream machine-readable consumers. On the **v2 platform track**, the Alpha-wave foundations have landed: a plugin host with declarative `plugin.yaml` manifests, default-deny permissions and a Python SDK; an agent registry and budgeted agent runtime with `/v2` catalog APIs; and the E0 infrastructure baseline (PostgreSQL state store, Redis, MinIO artifacts, OpenTelemetry) — see `docs/v2_platform/progress.md`. On the **Beta wave**, E11-S1 adds a self-hosted three-signal observability stack — OpenTelemetry Collector, Prometheus, Tempo, Loki, and a provisioned Grafana dashboard — via `make observability-up|verify|down`; see `docs/ops/observability.md`. E11-S2 adds real Control Plane authentication and RBAC — OIDC/JWKS bearer tokens, governed service credentials, browser sessions via PKCE login, a canonical five-role scope model enforced on every `/v2` route, and a durable, tenant-scoped access-audit trail — while keeping local zero-config access unchanged; see `docs/security.md`.
