...
### E0-S1 — Declarative, typed configuration layer

Subtasks:
- `E0-S1-T1`: config schema (Pydantic Settings) with local/prod profiles.
- `E0-S1-T2`: loading from env/file with precedence.
- `E0-S1-T3`: fail-fast validation + `config validate` command.
...
### E0-S2 — Migration to PostgreSQL as the default State Store

Subtasks:
- `E0-S2-T1`: initial modeling (sessions/runs/steps) with Alembic.
- `E0-S2-T2`: repository abstraction agnostic to SQLite/PostgreSQL.
- `E0-S2-T3`: reversible migration/seed.
...
### E0-S3 — Observability baseline (OpenTelemetry)

Subtasks:
- `E0-S3-T1`: request/step tracing.
- `E0-S3-T2`: metrics (counters/histograms) and OTLP exporter.
- `E0-S3-T3`: trace <-> run <-> step correlation.
...
### E0-S4 — Security baseline and secrets hygiene

Subtasks:
- `E0-S4-T1`: secret management (env/secret store) with no hardcoding.
- `E0-S4-T2`: secret scanning and SCA in CI.
- `E0-S4-T3`: default HTTP security headers.
...
### E0-S5 — Redis (Cache/Queue/Locks) and MinIO (Artifact Store)

Subtasks:
- `E0-S5-T1`: Redis connection with distributed locks.
- `E0-S5-T2`: MinIO/S3 client for artifacts.
- `E0-S5-T3`: local fallback without these dependencies.
