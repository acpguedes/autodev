...
| PostgreSQL persistence (State Store only) | `optional` | `backend/persistence/postgres_adapter/` `PostgresStore` — sessions/runs/messages/plans with migrations (E0-S3); selected via `DATABASE_URL=postgresql://…`; requires `psycopg`. **Partial (verified 2026-08-26):** covers the State Store, plus the eight dual-dialect stores (flows, events, artifacts, auth, plugins, repository indexing) sharing one dialect contract (`backend/persistence/contract.py`, E49, ADR-025) instead of eight hand-rolled copies. **`QuotaStore` is ported (E51, 2026-08-26)**, **`SecretStore` is ported (E52, 2026-08-26)**, **`PolicyStore` is ported (E53, 2026-08-26)**, **`EnvironmentStore` is ported (E54, 2026-08-26)**, and **`StepApprovalStore` (plan step state) is ported (E55, 2026-08-26)** — all five of the category-3 stores now run on both backends, with real cross-replica concurrency (advisory locks and state-guarded conditional updates, not a process-local lock) and tenant-scoped Row-Level Security. **Contract suite (E56, 2026-08-26):** `backend/tests/persistence_contract/` proves SQLite/PostgreSQL parity — CRUD, pagination, transactions/error equivalence, concurrency, tenant isolation, and migrations/restart — for every repository and store on a real PostgreSQL, not a monkeypatched one. The remaining E48-E60 program work (pooling, CI, data migration, backup) is tracked in `docs/v2_platform/postgres_production_completeness.md`. |
| Plan step approval (`/v2/plans`) | `stable` | `backend/api/routers/plan_approval_v2.py` + `backend/plans/step_state.py` `StepApprovalStore` — per-step `draft → under_review → approved \| rejected → executing → completed` state machine (E16-S2), tenant-scoped and dual-backend since E55. Every read/write threads the authenticated principal's `tenant_id` through the store; PostgreSQL enforces it via Row-Level Security (E50-S4) and a transaction-scoped advisory lock closes the same append/MAX(step_index) race E51/E52 closed for quota leases and secret rotations. A pre-E55 install's standalone `./autodev_plan_step_state.db` is migrated (not silently dropped) by `python -m backend.persistence.step_state_migration`. |
...
| pgvector semantic memory | `optional` | `backend/repository/embeddings/pgvector_store.py` + `backend/repository/embeddings/provider.py`; PostgreSQL + pgvector-backed embedding storage and top-k query (E7-S2/S3). The shipped Compose `prod`/`postgres` profiles use `pgvector/pgvector:0.8.3-pg16` (`infrastructure/docker-compose.yml:116`), and extension provisioning + preflight checks (`vector_provisioning.py`, `backend/ops/doctor.py`) fail closed with a named cause if the runtime cannot satisfy it — E48 |
...
| Approval gates blocking execution | `default` | Legacy `execute_plan()` still doesn't check plan status, but E14's governed execution pipeline gates on it: `backend/execution/executor.py` only skips the policy evaluator for an action whose id is in `pre_approved_action_ids` ("a human already explicitly approved this action"), backed by `backend/execution/policy.py` / `decisions.py` (E14-S3). **PostgreSQL (E53, 2026-08-26):** `PolicyStore` runs on both backends through the E49 contract; the pending-decision terminal transition (approve/reject) is a single state-guarded conditional `UPDATE`, exactly-once under concurrency, with idempotent replay of the same outcome. |
...
| Feature | Status | Notes |
|---------|--------|-------|
| Multi-tenant row-level isolation (PostgreSQL RLS) | `optional` | Mandatory `tenant_id` scoping on every `SessionRepository`/`RunRepository`/`MessageRepository`/`PlanRepository`/`EvalResultRepository`/`ScoreSnapshotRepository` method; `PostgresStore`/`PostgresPlanStore` additionally enforce Postgres Row-Level Security via `set_postgres_tenant()` (`backend/persistence/postgres_adapter.py`); ADR-010; requires the PostgreSQL backend. **Scope (updated 2026-08-26, E50):** RLS now covers all 24 tenant-scoped tables — the original 11 core/plan/code tables, plus the 13 domain tables (quotas, secrets, execution policy, environments, plan step state), versioned and RLS-enforced by `POSTGRES_STORE_MIGRATIONS` (`backend/persistence/migrations/postgres_versions.py`). **E51 (2026-08-26):** `QuotaStore`'s five tables (`tenant_quota_policies`, `tenant_usage_windows`, `run_leases`, `storage_reservations`, `request_rate_buckets`) are the first of the 13 to have their *store* actually read/write PostgreSQL, with RLS enforced per-operation via `set_postgres_tenant()`. **E52 (2026-08-26):** `secrets` is the second, with the same per-operation RLS scoping. **E53 (2026-08-26):** the four execution-policy tables (`execution_policy_rules`, `execution_dynamic_permissions`, `execution_policy_decisions`, `pending_action_decisions`) are the third, with the same per-operation RLS scoping — plus a fix for a latent gap where two pending-decision read/write paths took no `tenant_id` at all, which under RLS would have silently returned/affected zero rows for every tenant. **E54 (2026-08-26):** `execution_environments`/`execution_environment_decisions` are the fourth, with the tenant GUC now required on every lookup (not only writes) since both tables carry `FORCE ROW LEVEL SECURITY`. **E55 (2026-08-26):** `plan_step_state` is the fifth and last, closing out RLS enforcement across all 13 domain tables. |
...
| Tenant quota / run budget policy | `default` | `backend/quotas/{contracts,service}.py` (`TenantQuotaPolicy`, `RunBudgetLimits`, integer micro-USD amounts); `GET/PUT /v2/quotas/*` (`backend/api/routers/quotas_v2.py`); `QuotaExceededError` enforced on `POST /chat/dynamic` and other run-creating routes (E11-S3, ADR-019). **PostgreSQL (E51, 2026-08-26):** `QuotaStore` runs on both SQLite and PostgreSQL through the E49 contract — the `prod` profile can construct and use it; PostgreSQL concurrency (no oversubscription, no duplicate leases, no double-commit/double-refund) is `SELECT ... FOR UPDATE` plus a per-tenant `pg_advisory_xact_lock`, proven against a real multi-connection/multi-process PostgreSQL in `backend/tests/unit/quotas/test_postgres_concurrency.py`. |
...
| Real `prod`-profile E2E + backup/restore | `default` | `.github/workflows/ci-e2e.yml` `prod-e2e` job (E57, E59): boots the real `prod` profile, a session create/read flow + two-tenant RLS both directions + a real vector query (`scripts/ci_prod_e2e_smoke.py`), then a backup/wipe/restore round trip into a genuinely clean environment (database dropped/recreated, MinIO container replaced) with post-restore checks covering session listing, secret resolution (metadata + decrypted plaintext), artifact resolution, and a rerun vector query. Runs on every PR and on a daily schedule, measuring and recording RTO each run. |
| PostgreSQL backup coverage assertion + pgvector extension state | `default` | `backend/persistence/backup.py` `BackupManager` (E59-S1): diffs the live `public`-schema table list against the `pg_dump` archive's own table of contents and fails the backup closed if any live table is uncaptured (negative-control tested); manifest records the pgvector extension version and vector indexes, and restore refuses a target lacking the extension before `pg_restore` runs |
| Backup RPO deviation (periodic snapshots, not PITR) | `default` | [ADR-027](v2_platform/decisions/ADR-027-backup-rpo-periodic-base-backups.md) (E59-S3): RPO ≤ 5 min met by 5-minute logical snapshots for the Beta topology, not continuous WAL archiving; backup duration is timed and recorded (`autodev_backup_last_duration_seconds`) so the real worst-case RPO is measured, not assumed |
