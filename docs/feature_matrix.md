...
| PostgreSQL persistence (State Store only) | `optional` | `backend/persistence/postgres_adapter/` `PostgresStore` — sessions/runs/messages/plans with migrations (E0-S3); selected via `DATABASE_URL=postgresql://…`; requires `psycopg`. **Partial (verified 2026-08-26):** covers the State Store, plus the eight dual-dialect stores (flows, events, artifacts, auth, plugins, repository indexing) sharing one dialect contract (`backend/persistence/contract.py`, E49, ADR-025) instead of eight hand-rolled copies. `EnvironmentStore` still raises `ValueError` on a `postgresql://` URL — porting it onto the new contract is E54, tracked by E48-E60 (`docs/v2_platform/postgres_production_completeness.md`). **`QuotaStore` is ported (E51, 2026-08-26)**, **`SecretStore` is ported (E52, 2026-08-26)**, **`PolicyStore` is ported (E53, 2026-08-26)**, and **`StepApprovalStore` (plan step state) is ported (E55, 2026-08-26)** — four of the five category-3 stores now run on both backends, with real cross-replica concurrency (row locking plus a state-guarded conditional update, not a process-local lock) and tenant-scoped Row-Level Security on `plan_step_state`. |
